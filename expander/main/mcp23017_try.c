...
static void mcp23017_test_init()
{
    ...
    assert(i2c_bus != NULL);
    assert(device != NULL);
    assert(ESP_OK == mcp23017_check_present(device));
}
...
{
    uint8_t cnt = 5;
    assert(ESP_OK == mcp23017_set_io_dir(device, 0x00, MCP23017_GPIOA)); //A:OUTPUT, B:INPUT
    assert(ESP_OK == mcp23017_set_io_dir(device, 0xff, MCP23017_GPIOB));
    while (cnt--) {
        ...
        /*****Normal Test*****/
    	assert(ESP_OK == mcp23017_write_io(device, cnt, MCP23017_GPIOA));
